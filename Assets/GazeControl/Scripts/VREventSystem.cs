...
namespace GazeInput
{
    ...
    public class VREventSystem : MonoBehaviour
    {
        ...
        public VRInteractiveItem _CurrentInteractible;                //The current interactive item
        public VRInteractiveItem _LastInteractible;                   //The last interactive item
        ...
        IEnumerator preformClick()
        {
            ...
            if (_CurrentInteractible)
                ExecuteEvents.Execute(_CurrentInteractible.gameObject, eventSystem, ExecuteEvents.deselectHandler);
            ExecuteEvents.Execute(_CurrentInteractible.gameObject, eventSystem, ExecuteEvents.pointerExitHandler);
            yield return new WaitForEndOfFrame();
            if (_CurrentInteractible)
                ExecuteEvents.Execute(_CurrentInteractible.gameObject, eventSystem, ExecuteEvents.pointerDownHandler);
            yield return new WaitForEndOfFrame();
        ...
        void PlaceCursor()
        {
            if (_CurrentInteractible != null)
            {
                Debug.Log(_CurrentInteractible.transform.name);
