...
namespace I2.MiniGames
{
    ...
    public class PrizeWheel : MiniGame
    {
        ...
        [Rename("Anchor To Center")]
        public bool _AnchorSelectorToCenter = false;

        [Rename("Elastic Duration")]
        public float _SelectorElasticDuration = 0;
        [Rename("Elastic Amplitud")]
        public float _SelectorElasticAmplitud;
        [Rename("Elastic Period")]
        ...
        [Rename("Min Volume")]
        public float _Sound_KnotCollision_MinVolume = 0.2f;
        [Rename("Max Volume")]
        public float _Sound_KnotCollision_MaxVolume = 1;
        ...
        public float origyrotation = 0.0f;
        public float origxrotation = 0.0f;
        public float iselectory = 0.0f;
        public float iselectorx = 0.0f;
        ...
        public override void ApplyLayout()
        {
            ...
            if (_Selector)
            {
                float offsetAngle =(int)_SelectorDirection;

                if (_AnchorSelectorToCenter)
                {
                    //_Selector.localRotation = Quaternion.Euler(iselectorx, iselectory, 180 - offsetAngle);
                    //_Selector.localRotation = Quaternion.Euler(iselectorx, iselectory, 180 - offsetAngle);

                    //  _Selector.localPosition = Vector3.zero;
                    _Selector.localPosition = _Selector.localPosition;
                }
                else
                {
        ...
        void Start()
        {
            origyrotation = _Wheel.rotation.eulerAngles.y;
            origxrotation = _Wheel.rotation.eulerAngles.x;
            iselectory = _Selector.rotation.eulerAngles.y;
            iselectorx = _Selector.rotation.eulerAngles.x;
        ...
        private bool UpdateRotation(float initialAngle, float totalRotation, float initialTime, float totalTime, float speedUpTime)
        {
            ...
            {
                _Wheel.rotation = Quaternion.Euler(origxrotation, origyrotation, angle);
                //_Wheel.rotation = new Quaternion(0,0,angle,1); 

            }
            //_Wheel.rotation = new Quaternion(0, 0, _Wheel.rotation.z, 1);
            return dt >= 1;
        }
